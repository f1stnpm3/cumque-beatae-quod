# @f1stnpm3/cumque-beatae-quod

[![npm version](https://badge.fury.io/js/@f1stnpm3/cumque-beatae-quod.svg)](https://www.npmjs.com/package/@f1stnpm3/cumque-beatae-quod) [![travis ci](https://travis-ci.org/trenskow/@f1stnpm3/cumque-beatae-quod.svg?branch=master)](https://travis-ci.org/trenskow/@f1stnpm3/cumque-beatae-quod)
-

**@f1stnpm3/cumque-beatae-quod** is an asynchronous node.js library for validating and error correcting JavaScript data - which also includes JSON. It uses a very simple schema model - inspired by [Mongoose](https://npmjs.org/package/mongoose).
...
**@f1stnpm3/cumque-beatae-quod** uses a simple schema modal to specify how the data should be formatted. It supports generic validators for all types and type specific validators.

Usage:

````javascript
await @f1stnpm3/cumque-beatae-quod(dataToValidate, validationSchema, callback)
````
...
````javascript
const @f1stnpm3/cumque-beatae-quod = require('@f1stnpm3/cumque-beatae-quod');

@f1stnpm3/cumque-beatae-quod(inputData, {
...
try {
	data = await @f1stnpm3/cumque-beatae-quod(data, {
		'user': { type: String, required: true },
		'pass': { type: String, required: true }
	});
...
This is because **@f1stnpm3/cumque-beatae-quod** supports type shortcuts for all its supported types, and you are - if you want to help yourself - going to use them a lot. You can read more about [type shortcuts](#type-shortcuts) in the designated section at the near-bottom of this document.
...
If, though, any of the post validator functions throws an error, none of the rest of the post validators in the chain will get called, and *@f1stnpm3/cumque-beatae-quod* will throw the error as a `ValidationError`.
...
The others are a bit different. They are - in essence - a shortcut for the validator `type`. Instead of writing `type` you just specify the type directly. Available types are all the supported types of **@f1stnpm3/cumque-beatae-quod**, namely `Object`, `Array`, `String`, `Number`, `Boolean`, `Date` and custom types.
...
ISO-8601 is the date format that `JSON.stringify(...)` converts `Date` instances into, so this allows you to just serialize to JSON on - as an example - the client side, and then **@f1stnpm3/cumque-beatae-quod** will automatically convert that into a `Date` instance when validating on the server side.
...
* `@f1stnpm3/cumque-beatae-quod.validate.body(schema)` validates `req.body`.
* `@f1stnpm3/cumque-beatae-quod.validate.query(schema)` validates `req.query`.
* `@f1stnpm3/cumque-beatae-quod.validate.param(schema)` validates `req.param`.
* `@f1stnpm3/cumque-beatae-quod.validate.parameter(id, schema)` validates `req.param` as a route.

## Example

````javascript
const { validate } = require('@f1stnpm3/cumque-beatae-quod');
